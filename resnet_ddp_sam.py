...
from sam import SAM
from utils_bn import enable_running_stats, disable_running_stats
...
def train(dataloader, model, criterion, optimizer, scheduler, epoch, local_rank,
    ...
    for step, batch in enumerate(dataloader):
        ...
        criterion(model(samples), labels).backward()
...
def main(local_rank):
    ...
    momentum = 0.9
    weight_decay = 1e-4
    save_path = 'output/resnet_tddp_sam'
    ...
    optimizer = SAM(model.parameters(), SGD, rho=0.1, lr=lr, momentum=momentum, weight_decay=weight_decay)
